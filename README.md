...
- Then choose **GPU (Vulkan)** or **CPU only**, along with the speech model and language to start with; these default to Base (English) and English. Bantz downloads only the selected pinned Whisper.net runtime, verifies its SHA-256 hash, and extracts only the native files for the current platform. The choice remains editable in Settings; changing to a runtime that is not installed returns to setup on the next launch.
